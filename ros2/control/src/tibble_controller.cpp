...
{
    std::string TibbleController::modeToString(Mode m)
    {
        switch (m)
        {
    ...
    bool TibbleController::parseModeString(const std::string & s, Mode & out)
    {
    ...
    controller_interface::CallbackReturn TibbleController::on_configure(const rclcpp_lifecycle::State &)
    {
        ...
        RCLCPP_INFO(get_node()->get_logger(),
              "Configured TibbleController. Initial mode=%s",
    ...
    void TibbleController::cmdVelCb(const geometry_msgs::msg::Twist & msg)
    {
        CmdVel c;
    ...
    void TibbleController::digCmdCb(const std_msgs::msg::Float64MultiArray & msg)
    {
        DigCmd d;
    ...
    void TibbleController::setModeCb(const std_msgs::msg::String & msg)
    {
        Mode m;
    ...
    void TibbleController::latchFault(const std::string & reason)
    {
        if (!fault_latched_)
        {
            fault_latched_ = true;
    ...
    void TibbleController::clearFault()
    {
        fault_latched_ = false;
        mode_ = Mode::IDLE;
    ...
    bool TibbleController::isMovingNow() const
    {
    ...
    bool TibbleController::canTransition(Mode from, Mode to, bool moving_now) const
    {
        if (from == Mode::FAULT)
        {
    ...
    void TibbleController::enterMode(Mode new_mode)
    {
        mode_ = new_mode;
    ...
    void TibbleController::setWheelCommandsRadps(double left_radps, double right_radps)
    {
        // clamp to max
    ...
    void TibbleController::computeWheelRadps(double v_mps, double w_radps, double & out_left, double & out_right) const
    {
    ...
    loader_cmd_idx_].set_value(loader_cmd_val_);
        ...
        if (odom_pub_rt_ && odom_pub_rt_->trylock())
        {
            ...
            msg.twist.twist.linear.x =controller_interface::return_type TibbleController::update(const rclcpp::Time & time, const rclcpp::Duration & period)
    {
